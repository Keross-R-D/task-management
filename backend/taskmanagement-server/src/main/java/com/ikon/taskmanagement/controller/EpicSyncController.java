...
import com.ikon.taskmanagement.api.EpicSyncApi;
...
public class EpicSyncController implements EpicSyncApi {
    ...
    @Override
    public ResponseEntity<Map<String, String>> triggerEpicSync() {
        try {
            ...
            Optional<ConnectorConfigResponse> projectConfig = configService.getAllConnectorConfigs().stream()
                    .filter(c -> "Epic".equals(c.getAvailableModule()))
                    .max(Comparator.comparing(ConnectorConfigResponse::getCreatedAt)); // <-- Updated here

            if (projectConfig.isEmpty()) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND)
                        .body(Map.of("error", "Epic connector configuration not found."));
            }
            ...
            if (projectConnection.isEmpty()) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND)
                        .body(Map.of("error", "Epic connection not found."));
            }
            ...
            return ResponseEntity.accepted().body(Map.of(
                    "status", "ACCEPTED",
                    "message", "Epic sync triggered successfully. Running in the background."));

        } catch (Exception e) {
            log.error("Failed to trigger manual sync for epics: {}", e.getMessage(), e);
