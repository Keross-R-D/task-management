...
public interface SprintApi {
    ...
    @PostMapping
    ResponseEntity<SprintResponseDto> createSprint(@RequestBody SprintRequestDto dto);

    @Operation(summary = "Fetch sprints by Epic ID")
    @GetMapping("/epic/{epicId}")
    ResponseEntity<List<SprintResponseDto>> getSprintsByEpicId(@PathVariable("epicId") UUID epicId);

    @Operation(summary = "Fetch sprints by Project ID")
    @GetMapping("/project/{projectId}")
    ResponseEntity<List<SprintResponseDto>> getSprintsByProjectId(@PathVariable("projectId") UUID projectId);

    @Operation(summary = "Fetch sprint by ID")
    @GetMapping("/{id}")
    ResponseEntity<SprintResponseDto> getSprintById(@PathVariable("id") UUID id);

    @Operation(summary = "Update an existing sprint")
    @PutMapping("/{id}")
    ResponseEntity<SprintResponseDto> updateSprint(@PathVariable("id") UUID id, @RequestBody SprintRequestDto dto);
