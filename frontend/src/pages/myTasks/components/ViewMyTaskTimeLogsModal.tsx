...
import {
    ...
    type MyTaskWorklog,
} from "@/features/myTaskWorklogs/myTaskWorklogApiSlice";
import type { Task } from "@/features/myTasks/mytasksApiSlice";
...
interface FlattenedLog {
    ...
    description: string;
    createdAt?: string;
    originalWorklog: MyTaskWorklog;
}

export default function ViewMyTaskTimeLogsModal({
    open,
    task,
    onClose,
    onLogMoreTime,
}: Props) {
    const { data: worklogs = [], isLoading } = useGetWorklogsByMyTaskQuery(
        task?.id || "",
        {
            skip: !task?.id || !open,
        }
    ...
    const [updateWorklog] = useUpdateMyTaskWorklogMutation();
    const [deleteWorklog] = useDeleteMyTaskWorklogMutation();
    ...
    const progressPercent =
        estimated > 0
            ? Math.min(100, Math.round((totalLogged / estimated) * 100))
            : 0;
            ...
            <DialogContent className="max-w-lg bg-[#0b0c10] text-gray-100 border-gray-800" onInteractOutside={(e) => e.preventDefault()}>
                <DialogHeader className="my-2 border-b border-gray-800 pb-4">
                    ...
                    <span className="text-gray-400 text-sm">
                        Logged time entries for "{task?.taskTitle || "task"}"
                    </span>
                </DialogHeader>
