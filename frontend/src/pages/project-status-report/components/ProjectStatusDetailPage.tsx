...
import { Card, CardContent, CardHeader, CardTitle, DataTableLayout, Progress } from "ikon-react-components-lib";
import { ChevronLeft, FolderKanban, Info, TriangleAlert, Calendar, ChartColumn, Clock, Target, CircleCheck, CircleAlert } from "lucide-react";
...
const ProjectStatusDetailPage: React.FC = () => {
    ...
    const { data: sprints = [], isLoading, isError } = useGetSprintsByProjectQuery(id);
    ...
    //Columns for Overdue tasks table
    ...
    //Table data for Overdue tasks
    const overdueTasks = sprints.map((sprint) => {
    ...
    const getProjectStatus = (endDate: string, overdueSprints: number) => {
        ...
        // No overdue sprints means on time
    ...
    //Get current, previous and upcoming week range
    ...
    const upcomingWeek = getWeekRange(nextWeekDate);
    ...
    const isInRange = (date: Date, start: Date, end: Date) =>
        ...
        return (
            (start <= currentWeek.end && end >= currentWeek.start)
        );
    });
    ...
    const previousWeekSprints = sprints.filter((sprint) => {
        ...
        return isInRange(end, previousWeek.start, previousWeek.end);
    });

    const upcomingWeekSprints = sprints.filter((sprint) => {
        const end = new Date(sprint.endDate);

        return isInRange(end, upcomingWeek.start, upcomingWeek.end);
    });
    ...
    const previousWeekData = previousWeekSprints.map(s => ({
        sprintName: s.name,
        progress: getProgress(s.id),
    }));
    ...
    //Data table columns for current week
    const currentWeekColumns = [
        ...baseSprintColumns,
        {
            accessorKey: "progress",
            header: () => <div className="font-semibold">Progress</div>,
            cell: ({ row }: any) => renderProgressCell(row.original.progress),
        },
    ];

    //Data table columns for previous week
    ...
    //Data table columns for upcoming week
    const upcomingWeekColumns = [
        ...baseSprintColumns,
        {
            accessorKey: "plannedEndDate",
            header: () => <div className="font-semibold">Planned End Date</div>,
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2! x gap-6">
                    ...
                    <Card>
                        <CardHeader className="flex justify-between items-center">
                            ...
                            <span className={`px-3 py-1 text-xs rounded-full ${getStatusClass(status)}`}>
                                {status === "on_time" ? "On Time" : status === "slight_delay" ? "Slight Delay" : "Delay"}
                            </span>
                        </CardHeader>
                        ...
                        <CardContent className="space-y-3 text-sm">
                            ...
                            <div className="flex flex-wrap gap-4 text-xs pt-2">
                                <span className="flex gap-2 items-center justify-center font-semibold text-green-500"><span className="w-2 h-2 rounded-full bg-green-500" />On Time</span>
                                <span className="flex gap-2 items-center justify-center font-semibold text-yellow-500"><span className="w-2 h-2 rounded-full bg-yellow-500" />Slight Delay</span>
                                <span className="flex gap-2 items-center justify-center font-semibold text-red-500"><span className="w-2 h-2 rounded-full bg-red-500" />Delay</span>
                            </div>
                        </CardContent>
                    </Card>
